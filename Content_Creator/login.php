<?php
	...
	if(isset($_POST['login']))
	{
        ...
        $index_qry=mysqli_query($con,"select index_point as index_data from tbl_content_creator where  (username='".$username."' or email='".$username."') and password='".md5($password)."'");
        $index=mysqli_fetch_array($index_qry);
        $index=$index['index_data'];
        if($index<5)
        {
            mysqli_query($con,"update tbl_content_creator set Status=0 where (username='".$username."' or email='".$username."') and password='".md5($password)."'");
            $date_data=mysqli_query($con,"select SUBSTRING(u_date,0,10) as date from tbl_content_creator where (username='".$username."' or email='".$username."') and password='".md5($password)."'");
            $date=mysqli_fetch_assoc($date_data);
            $date1=date_create($date['date']);
            $date2=date_create(date("Y-m-d"));
            $diff=date_diff($date1,$date2);
            $diff=$diff->format("%a"); 
            echo $diff;
            echo "<script>alert('you are enable to login because your index point are below 15');</script>";
        }
        if($diff>=15)
        {   
            mysqli_query($con,"update tbl_content_creator set Status=1 and index_point=6");
        }
        if($index>=10)
        {
