...
if(isset($_POST['add_'.$type.'']))
{
  ...
  if($flag==true)
  {
    ...
    $new_can_insert=0;
    while($r=mysqli_fetch_array($count_news))
    {
      if($r['PostDate']==date('Y-m-d'))
      {
        $new_can_insert++;
      }
    }
...
{
  if(empty($_POST['channelname']) || empty($_POST['description']))
  {
    $error_channel="insert valid details";
  }
  else{
...
{ $temp=$details;
  ...
  if(empty($_POST["summary"]))
  {
    ...
    {
      $a=$_GET["newsid"];
      $sql='update tbl_news set CategoryID="'.$cat.'", CreatorID="'.$creatorid.'",HeadLine="'.$headLine.'",FileAttach="'.$newfilename.'",Summary="'.$summary.'",Status="'.$status.'",Details="'.$details.'",PostDate="'.$date.'" where id="'.$a.'"';
...
function CommentApprove($id)
{
  ...
  header("location:comment.php?comment");
}
