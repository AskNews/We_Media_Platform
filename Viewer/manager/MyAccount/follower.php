...
<li class="one_quarter push30">
    ...
    if($t<=$c)
    {
        $qry=mysqli_query($con,"select id,username from tbl_content_creator where id in(select content_creator_id from tbl_follower where viewer_id=".$uid.") limit ".$t.",".$rec);
        $t=$t+$rec;
        while($row=mysqli_fetch_array($qry))
        {
        ?>
            <li><a href="custom.php?id=<?php echo $row['id'];?>"><?php echo $row['username'] ?></a></li>
        <?php }
    } ?>
